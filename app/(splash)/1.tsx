...
export default function Splash1() {
  ...
  useEffect(() => {
    const id = setTimeout(() => router.replace('/(splash)/2'), 3500);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Image source={require('@/assets/images/hero-1.jpg')} style={styles.hero} resizeMode="cover" />
      <View style={styles.overlay}>
        <Text style={styles.subtitle}>{t('splash_discover')}</Text>
        <Text style={styles.tagline}>{t('splash_tagline')}</Text>
      </View>
      <Pressable style={styles.next} onPress={() => router.replace('/(splash)/2')}>
